...
* RCS dev started in the 1970s (text files only); moved gradually from a centralized to a distributed system (e.g., [SVN](http://en.wikipedia.org/wiki/Apache_Subversion) to [Git](http://learn.github.com/p/intro.html), which we'll be using in this seminar) 
